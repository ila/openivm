...
| `openivm_enable_data_dependent_optimizers` | BOOLEAN | `false` | Experimental: keep data-dependent optimizers (statistics_propagation) on while planning the delta template. Unsafe — folds current table contents (count→constant, predicate→empty, COALESCE→column) into the template, freezing/corrupting the delta |
