...
Native refreshes and after-hooks whose writes bind to the same database commit together. If an atomic after-hook fails, the refresh rolls back and can be retried. DuckLake refreshes and hooks that write to another database use staged execution; scripts whose write targets cannot be resolved before execution also use this path. OpenIVM persists a pending after-hook and retries it even when there are no remaining input changes. A process failure after the hook commits but before its pending flag is cleared can repeat that hook; use idempotent SQL for those hooks.

Dropping a tracked native source records retractions for its existing rows. Recreating it under the same name with a compatible schema then contributes new insertions, so retained MVs can refresh correctly. Replacing a native parent MV preserves pending consumer deltas and emits the old/new rows to its descendants. A replacement that changes the visible column names or types is rejected while dependent MVs exist.

Chained MVs consume a stable table containing their parent's visible rows and a hidden `openivm_ordinal` column. Ordered MVs publish a row number in their declared order; unordered MVs use zero. User-facing views hide this column, while ordered descendants bind to it so replacing a parent's sort expression does not leave the old ordering embedded in child SQL. Internal aggregate helpers, `HAVING`, and `ORDER BY ... LIMIT` are evaluated before publication. Replacing a parent with the same visible schema publishes the bag difference between its old and new output; children retain their incremental maintenance strategy and pending changes remain available to lagging consumers. Native publication commits with the refresh, and DuckLake publication retains the table identity used for snapshot tracking. For development databases created before this representation, recreate the MV chains from their original SQL to adopt the published boundary; legacy compiled definitions are not migrated automatically.

Publication stores a second copy of the visible output. Where the maintenance delta identifies affected rows or groups, publication compares only those keys; ordered publication and global operations such as `LIMIT` compare the visible result to preserve ordinals and select replacement rows correctly. The reproducible performance check is `python3 benchmark/poc/published_boundary_poc.py build/release/duckdb`; it records refresh timings and database sizes and verifies every MV with bidirectional `EXCEPT ALL` after each batch of conflicting changes. See the [measured performance comparison](published-boundary-performance.md).
