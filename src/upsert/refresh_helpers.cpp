...
struct StandardProjectionKeySource {
	string base_table;
	string delta_table;
};

static bool ResolveStandardProjectionKeySource(RefreshMetadata &metadata, const string &view_name,
                                               const vector<string> &delta_table_names, const string &table_name,
                                               const string &view_catalog_name, const string &view_schema_name,
                                               const string &attached_db_catalog_name,
                                               const string &attached_db_schema_name,
                                               StandardProjectionKeySource &out) {
	string metadata_key;
	for (auto &delta_name : delta_table_names) {
		string base_name = BaseTableNameFromDeltaKey(delta_name);
		if (StringUtil::CIEquals(base_name, table_name) ||
		    StringUtil::CIEquals(base_name, SqlUtils::LastIdentifierPart(table_name))) {
			metadata_key = delta_name;
			break;
		}
	}
	if (metadata_key.empty() || metadata.IsDuckLakeTable(view_name, metadata_key)) {
		return false;
	}
	...
	auto location = metadata.GetSourceLocation(view_name, metadata_key, fallback_catalog, fallback_schema);
	if (location.catalog_name.empty()) {
		location.catalog_name = fallback_catalog;
	}
	if (location.schema_name.empty()) {
		location.schema_name = fallback_schema.empty() ? "main" : fallback_schema;
	}
	if (location.catalog_name.empty()) {
		return false;
	}
	out.base_table =
	    SqlUtils::FullName(location.catalog_name, location.schema_name, SqlUtils::LastIdentifierPart(table_name));
	out.delta_table = metadata.ResolveDeltaQualifiedName(view_name, metadata_key, fallback_catalog, fallback_schema);
	return true;
}

static string BuildStandardProjectionLineageArmSQL(
    RefreshMetadata &metadata, const string &view_name, const vector<string> &delta_table_names,
    const RefreshMetadata::ProjectionKeyLineageArm &arm, const string &output_col, const string &view_catalog_name,
    const string &view_schema_name, const string &attached_db_catalog_name, const string &attached_db_schema_name) {
	StandardProjectionKeySource source;
	if (!ResolveStandardProjectionKeySource(metadata, view_name, delta_table_names, arm.source, view_catalog_name,
	                                        view_schema_name, attached_db_catalog_name, attached_db_schema_name,
	                                        source)) {
		return "";
	}
	string source_col = SqlUtils::QuoteIdentifier(arm.source_col);
	string lineage_key = SqlUtils::QuoteIdentifier("openivm_lineage_key");
	string sql = "SELECT " + source_col + " AS " + lineage_key + " FROM " + source.delta_table + " WHERE " +
	             SqlUtils::QuoteIdentifier(openivm::MULTIPLICITY_COL) + " <> 0";

	for (auto &step : arm.steps) {
		StandardProjectionKeySource lookup;
		if (!ResolveStandardProjectionKeySource(metadata, view_name, delta_table_names, step.table, view_catalog_name,
		                                        view_schema_name, attached_db_catalog_name, attached_db_schema_name,
		                                        lookup)) {
			return "";
		}
		string lookup_col = SqlUtils::QuoteIdentifier(step.lookup_col);
		string lookup_out = SqlUtils::QuoteIdentifier(step.lookup_out);
		string envelope = "(SELECT " + lookup_col + " AS openivm_lookup_col, " + lookup_out +
		                  " AS openivm_lookup_out FROM " + lookup.base_table + " UNION ALL SELECT " + lookup_col +
		                  " AS openivm_lookup_col, " + lookup_out + " AS openivm_lookup_out FROM " +
		                  lookup.delta_table + " WHERE " + SqlUtils::QuoteIdentifier(openivm::MULTIPLICITY_COL) +
		                  " < 0)";
		sql = "SELECT l.openivm_lookup_out AS " + lineage_key + " FROM (" + sql + ") c JOIN " + envelope +
		      " l ON l.openivm_lookup_col IS NOT DISTINCT FROM c." + lineage_key;
	}
	return "SELECT " + lineage_key + " AS " + SqlUtils::QuoteIdentifier(output_col) + " FROM (" + sql + ") openivm_k";
}
...
bool TryBuildSparkProjectionKeyRefresh(RefreshMetadata &metadata, const string &view_name,
	...
	bool has_repeated_source = false;
	for (idx_t i = 0; i < lineage.arms.size() && !has_repeated_source; i++) {
		for (idx_t j = i + 1; j < lineage.arms.size(); j++) {
			if (StringUtil::CIEquals(lineage.arms[i].source, lineage.arms[j].source) &&
			    lineage.arms[i].occurrence != lineage.arms[j].occurrence) {
	...
	vector<string> arms;
	for (auto &arm : lineage.arms) {
		string arm_sql = BuildStandardProjectionLineageArmSQL(metadata, view_name, delta_table_names, arm,
		                                                      lineage.output_col, view_catalog_name, view_schema_name,
		                                                      attached_db_catalog_name, attached_db_schema_name);
		if (arm_sql.empty()) {
			return false;
		}
		arms.push_back(std::move(arm_sql));
	}
	if (arms.empty()) {
		return false;
	}

	StandardProjectionKeySource key_source;
	if (!ResolveStandardProjectionKeySource(metadata, view_name, delta_table_names, lineage.key_source,
	                                        view_catalog_name, view_schema_name, attached_db_catalog_name,
	                                        attached_db_schema_name, key_source)) {
		return false;
	}
	string affected_temp = SqlUtils::QuoteIdentifier("openivm_affected_" + view_name);
	string old_temp = SqlUtils::QuoteIdentifier("openivm_old_" + view_name);
	string new_temp = SqlUtils::QuoteIdentifier("openivm_new_" + view_name);
	string qkey = SqlUtils::QuoteIdentifier(lineage.output_col);
	string affected_sql = "SELECT DISTINCT " + qkey + " FROM (" +
	                      StringUtil::Join(arms, arms.size(), " UNION ALL ", [](const string &arm) { return arm; }) +
	                      ") openivm_projection_keys";
	string restricted_source = "(SELECT * FROM " + key_source.base_table +
	                           " openivm_key_source WHERE EXISTS (SELECT 1 FROM " + affected_temp +
	                           " openivm_aff WHERE openivm_aff." + qkey + " IS NOT DISTINCT FROM openivm_key_source." +
	                           SqlUtils::QuoteIdentifier(lineage.key_col) + "))";
	bool replaced = false;
	string pushed_query = SqlUtils::ReplaceTableReferenceOccurrence(
	    view_query_sql, lineage.key_source, lineage.key_occurrence, restricted_source, replaced);
	if (!replaced || pushed_query == view_query_sql) {
		return false;
	}
	string target_match = "openivm_aff." + qkey + " IS NOT DISTINCT FROM openivm_target." + qkey;

	upsert_query = "CREATE OR REPLACE TEMP TABLE " + affected_temp + " AS\n" + affected_sql + ";\n\n";
	upsert_query += "CREATE OR REPLACE TEMP TABLE " + old_temp + " AS SELECT openivm_target.* FROM " + data_table +
