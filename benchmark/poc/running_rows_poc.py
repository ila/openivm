"""Compare affected-partition refresh with ROWS suffix maintenance; verify every batch."""
...
def main():
    ...
    parser.add_argument("before", help="Baseline binary; uses affected-partition maintenance")
    ...
    results = []
    binaries = [("before_partition", args.before, "false"), ("after_suffix", args.after, "true")]
        ...
        for label, binary, enabled in binaries:
            for repeat in range(3):
                sql = (
                    ...
                    f"CREATE TABLE t AS SELECT 1 k, i//2 d,(i//2)%17-8 x FROM range({size}) r(i); "
                    f"CREATE MATERIALIZED VIEW m AS {query};\n"
                )
                for batch in range(3):
                    start = size + batch * delta
                    sql += (
                        f"INSERT INTO t SELECT 1,i//2,(i//2)%17-8 FROM range({start},{start+delta}) r(i);\n"
                        ".timer on\nPRAGMA refresh('m');\n.timer off\n"
                ...
                results.append(
                    dict(size=size, delta=delta, label=label, repeat=repeat, seconds=times)
                )
